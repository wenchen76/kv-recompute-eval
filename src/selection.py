...
fresh-vs-stale deviation produced by the recompute itself, so selection has
to be interleaved with the layer-by-layer forward. See
``src.cache_ops.cacheblend_recompute_gradual`` for the unified implementation.
...
def select_positions(
    ...
    *,
    ...
    Returns a ``set[int]`` of global positions consumed by
    ``cacheblend_recompute``. ``hkvd_gradual`` is not handled here — its
    ranking signal is the per-layer fresh-vs-stale deviation produced
    during recompute, so it must be invoked end-to-end via
    ``cacheblend_recompute_gradual``.

    ``kv_gold`` / ``kv_stale`` are only required for ``hkvd``.
    ``seed`` is only used by ``random``.
    """
    if strategy == "random":
    ...
    if strategy == "hkvd_gradual":
        raise ValueError(
            "hkvd_gradual is dynamic — selection is interleaved with the "
            "layer-by-layer recompute. Call "
            "src.cache_ops.cacheblend_recompute_gradual directly instead "
            "of select_positions for this strategy."
        )
